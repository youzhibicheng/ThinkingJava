...
public class ArrayQueue<E> implements Cloneable
{
   ...
   private E[ ] data;
   private int manyItems; 	//存储数据的个数
   private int front;	//当前已被使用的数组的第一个下标
   private int rear;	//当前已使用数组中的最后一个下标,添加数据项,rear=rear+1,数据存储到data[rear]
   //一定要将数组看成环,这种数组叫循环数组,rear不一定一定大于front
   //不论何时,data[rear]都是队列中的最后一个数据项
   /**
   ...
   **/    
   public void add(E item)
   {
      ...
      if (manyItems == 0)
      {
         ...
         rear = nextIndex(rear);	//rear采用环形loop
   ...
   **/
   public void ensureCapacity(int minimumCapacity)
   {
      ...
      {  // Create a bigger array, but be careful about copying items into it. The queue items
         ...
         front = 0;
         rear = manyItems-1;//注意rear和manyItems的关系
         data = biggerArray;
      }
   }
