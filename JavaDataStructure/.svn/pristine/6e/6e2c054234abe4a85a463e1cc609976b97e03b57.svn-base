...
public class Table
{
   ...
   //      element抯 key is placed at keys[i].
   ...
   **/
   ...
   // value is the index of the specified key. Otherwise, the return value is -1.
   {
      ...
      int i = hash(key);
      //通过hash值直接定位
      //因为不同的key可能有相同的hash值,所以如果第一个不是的话,还是需要接着遍历的
   ...
   **/
   ...
   // The return value is a valid index of the table抯 arrays. The index is
   // calculated as the remainder when the absolute value of the key抯
   // hash code is divided by the size of the table抯 arrays.
   {
   ...
   *   the new element that抯 being added to this table
   * <dt><b>Precondition:</b><dd>
   *   If there is not already an element with the specified <CODE>key</CODE>,
   *   then this table抯 size must be less than its capacity 
   ...
   **/
   public Object put(Object key, Object element)
   {
      ...
      {  // The key is not yet in this Table.
    	  //如果不存在,将key存放在keys数组的hashCode位置
    	  //如果hashCode位置已存在,向后移动
         index = hash(key);
   ...
   **/
   public Object remove(Object key)
   {
      int index = findIndex(key);
      Object answer = null;
      //需不需要区别key=keys[i]?应该是要检测的,findIndex()里面进行了检测
      //因为不同的值可能产生相同的hashCode
      //也就是说hashCode相等,不代表值相等
      if (index != -1)
      {
         answer = data[index];
         keys[index] = null;
