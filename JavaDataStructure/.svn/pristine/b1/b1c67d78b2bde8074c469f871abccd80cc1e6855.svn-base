...
//(((6+9)/3)*(6-4))
//计算最内层表达式最左边的一个 
//1. 一个栈存放数值,这些数值包括输入的数值,也包括字表达式计算得到的数值
//2. 另一个栈存放需要被计算的运算符号
//数值栈取2个值,符号栈取1个符号,运算后得到值压入数值栈
...
public class EvaluateDemonstration {
	...
	// Postcondition: The top two numbers have been popped from the numbers
	// stack, and the
	// top operation has been popped from the operations stack. The two numbers
	// have been
	// combined using the operation (with the second number popped as the left
	// operand).
	// The result of the operation has then been pushed back onto the numbers
	// stack.
	// Exceptions: Throws an IllegalArgumentException if the stacks are illegal
	// or if the
