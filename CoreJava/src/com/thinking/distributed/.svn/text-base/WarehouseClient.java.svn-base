...
{  
   public static void main(String[] args)
   {
      try
      {  
         System.setProperty("java.security.policy", "client2.policy");
   ...
   {  
      ...
      {  
         ...
         Customer c = new Customer(Integer.parseInt(age.getText()),
            (male.isSelected() ? Product2.MALE : 0)
            + (female.isSelected() ? Product2.FEMALE : 0),
            selected.toArray(new String[selected.size()]));
         ArrayList<Product2> recommendations = warehouse.find(c);
         result.setText(c + "\n");
         for (Product2 p : recommendations)
         {  
