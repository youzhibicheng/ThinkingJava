...
public class GridBagPane extends JPanel
{
   ...
   */
   public GridBagPane(String filename)
   {
      ...
      try
      {
         ...
         if (filename.contains("-schema"))//XML Schema解析
         {
   ...
   private void parseGridbag(Element e)
   {
      ...
      for (int i = 0; i < rows.getLength(); i++)
      {
         ...
         for (int j = 0; j < cells.getLength(); j++)
         {
            ...
            parseCell(cell, i, j);//i表示是第几个row,j表示第几个cell,通过i,j2个值,就能够定位具体的cell了
         }
      }
   }
   ...
   */   
   private Object parseBean(Element e)
   {
      ...
      {
         NodeList children = e.getChildNodes();
         Element classElement = (Element) children.item(0);//bean <!ELEMENT bean (class, property*)>
         ...
         for (int i = 1; i < children.getLength(); i++)//逐个遍历property属性
         {
            Node propertyElement = children.item(i);//<!ELEMENT property (name, value)>
            Element nameElement = (Element) propertyElement.getFirstChild();//name
            String propertyName = ((Text) nameElement.getFirstChild()).getData();
            
            Element valueElement = (Element) propertyElement.getLastChild();//value
            Object value = parseValue(valueElement);//<!ELEMENT value (int|string|boolean|bean)>
